...
class RPlanhgDataset(Dataset):
        ...
        if os.path.exists(
            ...
            if self.non_manhattan:
                for h, graph in tqdm(zip(self.org_houses, self.org_graphs), desc='processing dataset'):
                    ...
                    print(f"h {graph}. type {type(graph)}")
            ...
            for index, (h, graph) in enumerate(tqdm(zip(self.org_houses, self.org_graphs), desc='processing dataset')):
                house = []
                corner_bounds = []
                num_points = 0
                print(f"h {h}. type {type(h)}")
                ...
                for h, graph in tqdm(zip(self.org_houses, self.org_graphs), desc='processing dataset'):
                    ...
                    while np.sum(num_room_corners_total) >= max_num_points:
                        num_room_corners_total = [
                            cnumber_dist[room[1]][random.randint(0, len(cnumber_dist[room[1]]) - 1)] for room in h]
                    for i, room in enumerate(h):
                        # Adding conditions
    ...
    def __getitem__(self, idx):
        ...
        # print(f"arr {nd_array_to_decimal(arr, 'arr')}")
        # print(f"houses {nd_array_to_decimal(self.houses[idx], 'houses')}")
        graph = np.concatenate((self.graphs[idx], np.zeros([200 - len(self.graphs[idx]), 3])), 0)
        # print(f"room_types {nd_array_to_decimal(self.houses[idx][:, self.num_coords:self.num_coords + 25], 'room_types')}")
        # print(f"room_indices {nd_array_to_decimal(self.houses[idx][:, self.num_coords + 57:self.num_coords + 89], 'room_index')}")
        # print(f"corner_indices {nd_arr ay_to_decimal(self.houses[idx][:, self.num_coords + 25:self.num_coords + 57], 'corner')}")
        # print(f"connections {nd_array_to_decimal(self.houses[idx][:, self.num_coords + 90:self.num_coords + 92], 'connection')}")
        cond = {
        ...
        if self.set_name == 'eval':
            ...
            # room_types = nd_array_to_decimal(self.syn_houses[idx][:, self.num_coords:self.num_coords + 25])
            # corner_indices = nd_array_to_decimal(self.syn_houses[idx][:, self.num_coords + 25:self.num_coords + 57])
            # room_indices = nd_array_to_decimal(self.syn_houses[idx][:, self.num_coords + 57:self.num_coords + 89])
            # connections = nd_array_to_decimal(self.syn_houses[idx][:, self.num_coords + 90:self.num_coords + 92])
            # np.set_printoptions(threshold=np.inf)
            # print(f"Room Type {room_types}"
            #       f"Corner Indices {corner_indices}"
            #       f"Room Indices {room_indices}"
...
def nd_array_to_decimal(nd_array, type_of):
    y = []
    print({type(nd_array[1])})
    print(nd_array[1])
    for x in nd_array:
        print(f"{type_of} {x}")
        x = [int(z) for z in x]
        try:
            decimal_integer = x.index(1)
        except ValueError:
            decimal_integer = 0
        y.append(decimal_integer)
